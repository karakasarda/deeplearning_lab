...
def safe_float(value, digits: int = 4) -> Optional[float]:
    if pd.isna(value):
        return None
...
def run_file(row: pd.Series, filename: str) -> str:
    path = Path(str(row["output_dir"])) / filename
    return str(path.resolve()) if path.exists() else ""


def confusion_pairs(cm_path: Path) -> List[Dict[str, object]]:
                ...
                pairs.append(
                    {
                        ...
                        "count": count,
                        "rate": safe_float(rate, 2),
                    }
                )
    return sorted(pairs, key=lambda item: item["count"], reverse=True)[:5]


def build_deck_data() -> Dict[str, object]:
    comparison_path = RESULTS_DIR / "final_model_comparison.csv"
    if not comparison_path.exists():
        raise RuntimeError("Run build_final_report.py first")

    comparison = pd.read_csv(comparison_path).sort_values("test_macro_f1", ascending=False)
    best = comparison.iloc[0]
    baseline = comparison[comparison["family"] == "linear_baseline"].sort_values(
        "test_macro_f1", ascending=False
    ).iloc[0]

    audit_path = RESULTS_DIR / "audit_summary.json"
    audit = json.loads(audit_path.read_text(encoding="utf-8")) if audit_path.exists() else {}
    clean_rows = int(audit.get("clean_rows", 0) or 0)
    class_distribution = audit.get("class_distribution", {})
    source_distribution = audit.get("source_distribution", {})
    text_length = audit.get("text_length", {})

    hpo_path = RESULTS_DIR / "hpo" / "hpo_best.json"
    hpo = json.loads(hpo_path.read_text(encoding="utf-8")) if hpo_path.exists() else {}
    hpo_summary_path = RESULTS_DIR / "hpo" / "hpo_summary.csv"
    hpo_summary = pd.read_csv(hpo_summary_path) if hpo_summary_path.exists() else pd.DataFrame()

    per_class_path = Path(str(best["output_dir"])) / "per_class_report.csv"
    per_class = pd.read_csv(per_class_path) if per_class_path.exists() else pd.DataFrame()
    policies_path = Path(str(best["output_dir"])) / "threshold_policies_test.csv"
    policies = pd.read_csv(policies_path) if policies_path.exists() else pd.DataFrame()
    cm_path = Path(str(best["output_dir"])) / "confusion_matrix.csv"

    model_rows: List[Dict[str, object]] = []
    for _, row in comparison.head(9).iterrows():
        model_rows.append(
            {
                "run": row["run_name"],
                "model": row["model"],
                ...
                "family": row["family"],
                "accuracy": safe_float(row["test_accuracy"], 4),
                "macroF1": safe_float(row["test_macro_f1"], 4),
                "prAuc": safe_float(row["test_macro_pr_auc"], 4),
                "ece": safe_float(row["test_top_label_ece"], 4),
                "classwiseEce": safe_float(row["test_classwise_ece"], 4),
            }
        )

    class_rows = [
        {"label": label, "count": int(count), "share": pct(int(count), clean_rows)}
        for label, count in class_distribution.items()
    ]
    source_rows = [
        {"source": source, "count": int(count), "share": pct(int(count), clean_rows)}
        for source, count in source_distribution.items()
    ]

    hpo_rows = []
    if not hpo_summary.empty:
        for _, row in hpo_summary.iterrows():
            hpo_rows.append(
                {
                    "trial": int(row["trial"]),
                    "lr": f"{float(row['learning_rate']):.0e}",
                    "cw": "yes" if bool(row["class_weight"]) else "no",
                    "valF1": safe_float(row["val_macro_f1"], 4),
                    "ece": safe_float(row["val_classwise_ece"], 4),
                    "time": f"{float(row['runtime_seconds']):.0f}s",
                }
            )
    ...
    for _, row in per_class.iterrows():
        per_class_rows.append(
            {
                "label": row.get("label"),
                "precision": safe_float(row.get("precision"), 3),
                "recall": safe_float(row.get("recall"), 3),
                "f1": safe_float(row.get("f1", row.get("f1-score")), 3),
        ...
        policy_rows.append(
            {
                ...
                "threshold": safe_float(row.get("threshold"), 2),
                "precision": safe_float(row.get("precision"), 4),
                "recall": safe_float(row.get("recall"), 4),
                "f1": safe_float(row.get("f1"), 4),
                "flagged": pct(float(row.get("flagged_rate", 0)), 1.0),
            }
        )
    ...
    calibration_rows = [
        {
            "model": row["model"],
            "topECE": safe_float(row["test_top_label_ece"], 4),
            "classECE": safe_float(row["test_classwise_ece"], 4),
            "logLoss": safe_float(row["test_log_loss"], 4),
        }
        for _, row in comparison.head(6).iterrows()
    ]

    return {
        "title": "Birleştirilmiş Türkçe Toksisite Verisinde Model Güvenilirliği",
        "subtitle": "Macro-F1, kalibrasyon ve eşik seçimi accuracy'den daha açıklayıcı.",
        "claim": "En iyi skor tek başına yeterli değil; dengesiz sınıflarda güvenilir karar için macro-F1, confusion matrix, ECE ve threshold politikası birlikte okunmalı.",
        "bestRun": best["run_name"],
        "bestModel": best["model"],
        "bestMacroF1": safe_float(best["test_macro_f1"], 4),
        "bestAccuracy": safe_float(best["test_accuracy"], 4),
        "bestPrAuc": safe_float(best["test_macro_pr_auc"], 4),
        "bestEce": safe_float(best["test_top_label_ece"], 4),
        "bestClasswiseEce": safe_float(best["test_classwise_ece"], 4),
        "baselineRun": baseline["run_name"],
        "baselineMacroF1": safe_float(baseline["test_macro_f1"], 4),
        "macroLift": safe_float(best["test_macro_f1"] - baseline["test_macro_f1"], 4),
        "datasetRows": f"{clean_rows:,}".replace(",", "."),
        "textMedian": safe_float(text_length.get("median", 0), 0),
        "textP95": safe_float(text_length.get("p95", 0), 0),
        "classRows": class_rows,
        "sourceRows": source_rows,
        "modelRows": model_rows,
        "hpo": {
            **hpo,
            "learning_rate_display": f"{float(hpo.get('learning_rate', 0)):.0e}" if hpo else "n/a",
        },
        "hpoRows": hpo_rows,
        "calibrationRows": calibration_rows,
        "confusionPairs": confusion_pairs(cm_path),
        "classDistributionImage": str((RESULTS_DIR / "plots" / "class_distribution.png").resolve()),
        "sourceDistributionImage": str((RESULTS_DIR / "plots" / "source_distribution.png").resolve()),
        "comparisonImage": str((RESULTS_DIR / "plots" / "final_model_comparison.png").resolve()),
        "bestConfusionImage": run_file(best, "confusion_matrix.png"),
        "bestConfusionNormalizedImage": run_file(best, "confusion_matrix_normalized.png"),
        "bestPerClassImage": run_file(best, "per_class_f1.png"),
        "bestReliabilityImage": run_file(best, "reliability_test.png"),
        "bestThresholdImage": run_file(best, "threshold_sweep_val.png"),
        "perClassRows": per_class_rows,
        "policyRows": policy_rows,
        "appendixMatrices": [
            {"run": row["run_name"], "path": run_file(row, "confusion_matrix.png")}
            for _, row in comparison.iterrows()
            if run_file(row, "confusion_matrix.png")
        ],
    }


def write_deck_script() -> None:
    script = r"""
...
"""
    (PRESENTATION_DIR / "build_deck.mjs").write_text(script.strip() + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    (PRESENTATION_DIR / "deck_data.json").write_text(
        json.dumps(deck_data, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    write_deck_script()
    print(f"Wrote {PRESENTATION_DIR / 'deck_data.json'}")
    print(f"Wrote {PRESENTATION_DIR / 'build_deck.mjs'}")
